...
::: check-in
### Check-in 4.2: Practice with Pivoting & Joins {#checkin4-2 .unnumbered}
...
prof_info <- data.frame(
  professor =
    c("Bodwin", "Glanz", "Carlton", "Sun", "Robinson","Williams"),
  undergrad_school =
    c("Harvard", "Cal Poly", "Berkeley", "Harvard", "Winona State University","UMBC"),
  grad_school =
    c("UNC", "Boston University", "UCLA", "Stanford", "University of Nebraska-Lincoln", "Rutgers")
)

prof_course <- data.frame(
  professor = c("Bodwin", "Glanz", "Carlton", "Theobold", "Robinson", "Williams"),
  Stat_331 = c(TRUE, TRUE, TRUE, TRUE, TRUE,TRUE),
  Stat_330 = c(FALSE, TRUE, TRUE, FALSE, TRUE,FALSE),
  Stat_431 = c(TRUE, TRUE, FALSE, TRUE, FALSE,TRUE),
  Cs_123 = c(FALSE, FALSE, FALSE, FALSE, FALSE, TRUE)
)

course_info <- data.frame(
  course = c("Stat_331", "Stat_330", "Stat_431", "Cs_123"),
  num_sections = c(8, 3, 1, 5)
)
```
...
```{r}
#| echo: false
inner_join(prof_info, prof_course, by = "professor") |>
  kable()
```
...
prof_course |>
  pivot_longer(cols = c(Stat_331, Stat_330, Stat_431, Cs_123),
               names_to = "course",
               values_to = "can_teach") |>
...
Visit [PA 4: Government Spending](https://gato365.github.io/stat331-calpoly-gato365/practice-activities/PA4-government-spending.html) for instructions.
